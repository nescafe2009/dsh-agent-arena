...
function validateMeetingInput(raw) {
	if (raw === null || typeof raw !== "object" || Array.isArray(raw)) throw new TypeError("请求体必须是 JSON 对象");
	...
	if (sourceParticipants.length < 2 || sourceParticipants.length > 4) throw new TypeError("参会 AI 数量必须在 2 到 4 个之间");
...
function parseSpeechDirectives(text, profiles) {
	...
	remainder = remainder.replace(/大家|所有人|所有AI|全部AI|你们|全员/gi, "").replace(/请|麻烦|让|叫|我|你|他|她|它|就|也|再|先|一下|暂时|现在|已经|了|吧|哦|哈/g, "").replace(/[\s，。！？、,.!?:：；;~～“”"'（）()]+/g, "");
	return {
...
function apply(ctx, config = {}) {
	...
	function updateWorkspaceArtifact(meeting, raw) {
		...
		if (Object.hasOwn(raw ?? {}, "status")) {
			...
			if (![
				...
				"accepted",
				"rejected"
			].includes(status)) throw new HttpError(400, "成果状态无效");
	...
	function normalizeCoordinationFile(value) {
		...
		return {
			key: process.platform === "win32" ? absolute.toLocaleLowerCase() : absolute,
			path: absolute
		};
	}
	...
	function claimRoleFiles(runtime, profile, requestedFiles) {
		...
		setRoleActivity(runtime.container, profile, {
			...
			stage: "已锁定文件，准备编辑",
			claimedFiles
		}, files.length ? `锁定 ${files.length} 个文件` : "");
		return {
			ok: true,
			files,
			conflicts: []
				...
				if (action === "update") {
					...
					if (ok) {
						...
						setRoleActivity(runtime.container, profile, {
							status: "waiting",
							stage: "等待文件锁",
							detail: `${owners} 正在编辑冲突文件`
						}, `检测到文件冲突：${owners}`, "warning");
					...
					setRoleActivity(runtime.container, profile, {
						...
						stage: `任务“${task.title}”：${task.status}`,
						detail: task.description
					}, `更新任务：${task.title} → ${task.status}`, task.status === "blocked" ? "warning" : "info");
					...
					setRoleActivity(runtime.container, profile, {
						status: "working",
						stage: `已登记成果：${artifact.title}`,
						detail: artifact.location || artifact.description
					}, `登记成果：${artifact.title}`, "success");
			...
			for (const file of files) {
				...
				if (claim) {
					setRoleActivity(runtime.container, profile, {
						status: "waiting",
						stage: "检测到编辑冲突",
						detail: `${claim.ownerName} 正在编辑 ${file.path}`
					}, `阻止了冲突编辑：${file.path}`, "warning");
		...
		const scan = () => {
			for (const event of handle.agent.session.events) {
				...
				if (event.type === "assistant/chunk") {
					...
					}, `${activity.stage}${activity.detail ? `：${activity.detail}` : ""}`, "tool", event.time);
					...
					if (activity) setRoleActivity(runtime.container, profile, {
						status: "working",
						stage: event.data?.message?.isError ? `${activity.label} 执行失败` : `正在分析 ${activity.label} 的结果`,
						currentTool: ""
					}, `${activity.label}${event.data?.message?.isError ? " 执行失败" : " 已完成"}`, event.data?.message?.isError ? "error" : "success", event.time);
					...
					setRoleActivity(runtime.container, profile, {
						status: "working",
						stage: event.data?.isError ? `${activity.label} 执行失败` : `正在分析 ${activity.label} 的结果`,
						currentTool: ""
					}, `${activity.label}${event.data?.isError ? " 执行失败" : " 已完成"}`, event.data?.isError ? "error" : "success", event.time);
				} else if (event.type === "assistant/message") setRoleActivity(runtime.container, profile, {
					status: "working",
					stage: "已生成阶段回复",
					currentTool: ""
				}, "生成了一条阶段回复", "message", event.time);
			}
		};
		scan();
	...
	async function runFullAgentTurnOnce(handle, prompt, runtime, profile, phase = "work") {
		...
		setRoleActivity(runtime.container, profile, {
			status: "working",
			stage: phase === "ack" ? "已确认，准备正式工作" : "本轮工作已完成",
			currentTool: ""
		});
		return {
			...outcome,
	...
	async function applyMeetingAdminAction(meeting, runtime, result) {
		if (result.action === "change-topic") if (result.topic.length < 2) appendSystem(meeting, "管理员没有识别到有效的新话题，请用“把话题改为：……”再试一次。", true);
		else {
	...
	async function evaluateMeetingContinuation(meeting, completedIds, runtime, requirePeerReaction = false) {
		...
		try {
			...
			if (decision.complete || !decision.onTopic) {
				appendSystem(meeting, decision.onTopic ? "当前任务已自然告一段落，AI 成员正在等待你的新消息。" : "接话方向开始偏离人类当前焦点，已停止 AI 自动接话并等待你的下一步指示。", true);
				return;
			}
	...
	async function actOnMeeting(meeting, body) {
		...
		if (action === "pause") {
			...
			} else throw new HttpError(409, "当前工作状态已经变化，请稍后重试");
		...
		} else if (action === "decision-reopen") appendSystem(meeting, `人类用户重开了决策“${reopenWorkspaceDecision(meeting, body).title}”。`, true);
	...
	async function createRoom(raw) {
		...
		if (type === "direct" && profileIds.length !== 1 || type === "group" && (profileIds.length < 2 || profileIds.length > 12)) throw new HttpError(400, type === "direct" ? "私聊必须选择 1 个 AI 用户" : "群聊必须选择 2 到 12 个 AI 用户");
	...
	async function deleteMeeting(meeting) {
		if (BUSY_MEETING_STATUSES.has(meeting.status) || runtimes.has(meeting.id)) throw new HttpError(409, "AI 正在工作，请先停止当前工作再删除会议");
		pendingMeetingStarts.delete(meeting.id);
